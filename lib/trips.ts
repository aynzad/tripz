...
export function calculateExpensesPerNight(
  expenses: Expenses,
  startDate: Date,
  endDate: Date
): number {
...
export function getUniqueCountries(trips: Trip[]): string[] {
  ...
  trips.forEach((trip) =>
    trip.destinations.forEach((d) => countries.add(d.country))
  );
...
export async function getTripsByCity(cityName: string): Promise<Trip[]> {
  ...
  return allTrips.filter((trip) =>
    trip.destinations.some((dest) => dest.city === cityName)
  );
}
 ...
 */
export async function getCityNameFromUrl(
  urlName: string
): Promise<string | null> {
  const allTrips = await getAllTrips();
