...
export default function TripDetailMap({ trip }: TripDetailMapProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-full overflow-hidden rounded-xl bg-background"
    >
      {/* Map Container */}
      <div
      ...
      >
        ...
        {trip.destinations.map((dest, index) => {
          ...
          return (
            ...
            >
              <div
                className="rounded-full border-2 border-white shadow-lg box-border"
                style={{
      ...
      <div className="absolute bottom-4 left-4 bg-card/90 backdrop-blur rounded-lg p-3 z-40 border border-border/50">
        ...
        <div className="flex flex-wrap gap-2">
          {Array.from(
            new Set(
              trip.destinations.map((d) => d.transportationType).filter(Boolean)
            )
          ).map((type) => (
