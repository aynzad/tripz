...
export default function TripMap({ trips, selectedTripId, onTripSelect }: TripMapProps) {
  ...
  const routeData = useMemo(() => {
    return trips.map((trip) => {
      ...
      const segments = trip.destinations.slice(0, -1).map((dest, i) => {
        ...
        const color = TRANSPORTATION_COLORS[nextDest.transportationType ?? 'default'] || TRANSPORTATION_COLORS.default
  ...
  }, [trips, selectedTripId, projectPoint, zoom])
  ...
  const markerData = useMemo(() => {
    return trips.flatMap((trip) => {
      ...
      return trip.destinations.map((dest, index) => {
        ...
        const color = TRANSPORTATION_COLORS[dest.transportationType ?? 'default'] || TRANSPORTATION_COLORS.default
    ...
    <div ref={containerRef} className="bg-background relative h-full w-full overflow-hidden">
        ...
        {markerData.map((marker) => (
          <motion.div
            ...
            onClick={(e) => {
              ...
              e.stopPropagation()
              onTripSelect(marker.trip.id === selectedTripId ? null : marker.trip.id)
            }}
          >
            <div
              className="box-border rounded-full border-2 border-white shadow-lg transition-transform hover:scale-125"
              style={{
      ...
      <AnimatePresence>
        {hoveredDestination && (
          ...
          >
            <div className="glass border-border/50 min-w-[280px] rounded-xl border p-4 shadow-2xl">
              <div className="relative -mx-4 -mt-4 mb-3 h-32 overflow-hidden rounded-t-xl">
                ...
                <div className="absolute right-3 bottom-2 left-3">
                  <h3 className="text-lg font-semibold text-white">{hoveredDestination.destination.city}</h3>
                  <p className="text-sm text-white/80">{hoveredDestination.destination.country}</p>
                </div>
              </div>
      ...
      <AnimatePresence>
        {selectedTripId && (
          <motion.button
            ...
            className="glass hover:bg-secondary/50 absolute top-4 left-4 z-40 flex items-center gap-2 rounded-lg px-4 py-2 transition-colors"
          >
            <X className="h-4 w-4" />
